...
SurfaceMeshGL::SurfaceMeshGL()
{
    ...
    front_color_ = vec3(0.125, 0.5, 1.0);
    back_color_ = vec3(0.0, 2.0/3, 0.0);
    ambient_ = 0.1;
    diffuse_ = 0.8;
    specular_ = 0.6;
    shininess_ = 100.0;
...
void SurfaceMeshGL::update_opengl_buffers()
{
    ...
    if (n_faces())
    {
        ...
        // drb
        position_array.reserve(3 * n_faces());
        auto faceVirtuals = get_face_property<Eigen::Vector3d>("faceVirtual");
        Eigen::Vector3d virtualVertex = {0,0,0};
        for(pmp::Face f : faces())
        {
            if(faceVirtuals)
            {
                position_array.push_back(faceVirtuals[f]);
            } else
            {
                virtualVertex *= 0;
                for (pmp::Vertex v : vertices(f))
                {
                    for (int d = 0; d < 3; d++)
                    {
                        virtualVertex[d] += position(v)[d];
                    }
                }
                position_array.push_back(virtualVertex / valence(f));
            }
        }
    ...
    if (!position_array.empty())
    {
        ...
        n_vertices_ = position_array.size() - n_faces();
    }
    else
    {
                         ...
                         const std::string& draw_mode)
{
    ...
    else if (draw_mode == "Hidden Line")
    {
        if (n_faces())
        {
            ...
            glEnable(GL_PROGRAM_POINT_SIZE);
            #endif
            glDrawArrays(GL_POINTS, 0, n_vertices_ + n_faces());

               ...
               const IOFlags& flags)
{
    ...
    // drb: write MeanRatioQuality
    if(mesh.has_face_property("MeanRatioQuality"))
    {
        auto meanRatioQualities = mesh.get_face_property<double>("MeanRatioQuality");
        for (auto f : mesh.faces())
        {
            double q = meanRatioQualities[f];
            fprintf(out, "q %.10f\n", q);
        }
    }
